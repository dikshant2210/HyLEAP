...
test_annotations = 'data/testing annotations/'
...
def get_test_batches(x_data_test, y_data_test, input_seq, output_seq, test_samples):
    ...
    x_batch = np.reshape(x_batch, (x_data_test.shape[0] * test_samples, input_seq, 2))

    y_batch = np.reshape(y_batch, (y_data_test.shape[0] * test_samples, output_seq, 2))
...
def bms_loss( y_true, y_pred):
    ...
    y_true = K.reshape( y_true, (batch_size,train_samples,predicting_frame_num,2) )
    y_pred = K.reshape( y_pred, (batch_size,train_samples,predicting_frame_num,2) )
...
def train(model, epochs, batch_gen, input, batch_size):
    ...
    model.fit_generator(batch_gen, steps_per_epoch=(int(input.shape[0] / batch_size)), epochs=epochs)
...
if __name__ == '__main__':
    ...
    (x_batch_test, y_batch_test) = get_test_batches(input_test, output_test,
                                                    observed_frame_num, predicting_frame_num, test_samples)

    batch_gen = get_batch_gen(input_train, output_train, batch_size,
                              observed_frame_num, predicting_frame_num, train_samples)
